...
class EncoderDecoder(chainer.Chain):
   def __init__(self):
      ...
      with self.init_scope():
         ...
         self.conv7_1 = L.Convolution2D(self.out_c*16, self.out_c*8, ksize=5, stride=1, pad=1)
         self.bn4     = L.BatchNormalization(self.out_c*8)
         self.dconv4  = L.Deconvolution2D(None, self.out_c*8, ksize=3)

         self.conv8_1 = L.Convolution2D(self.out_c*16, self.out_c*4, ksize=5, stride=1, pad=1)
         self.bn3     = L.BatchNormalization(self.out_c*4)
         self.dconv3  = L.Deconvolution2D(None, self.out_c*4, ksize=5, pad=1)

         self.conv9_1 = L.Convolution2D(self.out_c*8, self.out_c*2, ksize=5, stride=1, pad=1)
         self.bn2     = L.BatchNormalization(self.out_c*2)
         self.dconv2  = L.Deconvolution2D(None, self.out_c*2, ksize=3)

         self.convf_1 = L.Convolution2D(self.out_c*4, self.out_c*1, ksize=5, stride=1, pad=1)
         self.bn1     = L.BatchNormalization(self.out_c*1)
         self.dconv1  = L.Deconvolution2D(None, self.out_c*1, ksize=3)

         self.conv0_1 = L.Convolution2D(self.out_c*2, self.out_c, ksize=5, stride=1, pad=1)
         self.bn0     = L.BatchNormalization(self.out_c)

         self.convv   = L.Convolution2D(self.out_c, 1, ksize=5, stride=1, pad=1)

   def __call__(self, x):
      x = F.relu(self.conv1_1(x))
      c1 = F.relu(self.conv1_2(x))
      x = F.max_pooling_2d(c1, ksize=2, stride=2)

      x = F.relu(self.conv2_1(x))
      c2 = F.relu(self.conv2_2(x))
      x = F.max_pooling_2d(c2, ksize=2, stride=2)

      x = F.relu(self.conv3_1(x))
      x = F.relu(self.conv3_2(x))
      c3 = F.relu(self.conv3_3(x))
      x = F.max_pooling_2d(c3, ksize=2, stride=2)

      x = F.relu(self.conv4_1(x))
      x = F.relu(self.conv4_2(x))
      c4 = F.relu(self.conv4_3(x))
      x = F.max_pooling_2d(c4, ksize=2, stride=2)

      x = F.relu(self.conv5_1(x))
      x = F.relu(self.conv5_2(x))
      c5 = F.relu(self.conv5_3(x))
      x = F.max_pooling_2d(c5, ksize=2, stride=2)
